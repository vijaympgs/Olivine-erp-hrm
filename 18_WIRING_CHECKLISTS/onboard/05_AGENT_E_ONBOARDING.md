...
#### **Toolbar Implementation Guide** ⭐ **CRITICAL - UPDATED 2026-01-09**
...
**Key Content**:
- ✅ Character-based config string logic (e.g., `NESCKVDXRQFIO`)
- ✅ `MasterToolbar` component integration
- ✅ Backend model registry schema
- ✅ VIEW ↔ CREATE ↔ EDIT mode state machine
- ✅ **NEW**: Mode-based button filtering (not separate configs!)

**Additional References** (Read These!):
1. **`.steering/18_WIRING_CHECKLISTS/07_TOOLBAR_CODE_EXAMPLES.md`** ⭐ **START HERE!**
   - **COMPLETE copy-paste code examples**
   - Employee Master example (full implementation)
   - Leave Request example (transaction with workflow)
   - All config strings explained
   - Common patterns for HRM/CRM
   - **NO IMAGES - Pure code you can copy!**

2. `.steering/20TOOLBAR_ROLLOUT/01_ESSENTIAL/ARCHITECTURE_CLARIFICATION.md`
   - Explains single-entry-per-screen rule
   - Shows wrong vs correct approach
   
3. `.steering/20TOOLBAR_ROLLOUT/02_REFERENCE/MODE_BASED_FILTERING_TECHNICAL_REFERENCE.md`
   - Technical deep-dive on how component filters buttons
   - Filtering matrix for all modes
   - Real-world examples
...
### **❌ DO NOT Create Separate List View Entries**
- ❌ Don't create separate `ERPMenuItem` for list pages
- ❌ Don't use `view_type: "LIST_VIEW"` (removed from system!)
- ❌ Don't create multiple entries for same screen

**✅ INSTEAD**: 
- Create ONE `ERPMenuItem` entry per screen
- Use FULL config string (e.g., `NESCKZTJAVPMRDX1234QF`)
- Frontend handles list vs form via `mode` prop
- **Reference**: `.steering/20TOOLBAR_ROLLOUT/01_ESSENTIAL/ARCHITECTURE_CLARIFICATION.md`
...
- ❌ Don't hardcode `allowedActions` array

**✅ INSTEAD**: 
- For transaction forms: Use `TransactionToolbar` component
- For master pages: Use `MasterToolbar` component
- Let backend drive toolbar via `applicable_toolbar_config` string
- Use `mode` prop to control button visibility
