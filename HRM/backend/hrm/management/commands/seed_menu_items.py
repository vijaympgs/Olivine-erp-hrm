...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for item_data in menu_items:
            # Map fields to Core model schema
            defaults = {
                'menu_name': item_data.get('menu_name'),
                'module_name': item_data.get('module'), # Map module -> module_name
                'submodule': item_data.get('submodule'),
                'view_type': 'MASTER' if 'MST' in item_data.get('menu_type', '') else 
                             'TRANSACTION' if 'TXN' in item_data.get('menu_type', '') else 
                             'REPORT' if 'R' == item_data.get('menu_type') else 
                             'DASHBOARD' if 'D' == item_data.get('menu_type') else 'TRANSACTION',
                'toolbar_config': item_data.get('toolbar_config'),
                'applicable_toolbar_config': item_data.get('toolbar_config'), # Sync both
