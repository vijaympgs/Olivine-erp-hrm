...
# ROOT CAUSE ANALYSIS:
# Django is looking for 'HRM.backend.hrm' module but the settings.py is in hrm.backend
# The INSTALLED_APPS includes 'HRM.backend.hrm' which refers to the hrm package
...
def load_seed_data():
    ...
    for fixture_file in all_files:
        try:
            ...
            execute_from_command_line(['manage.py', 'loaddata', '--traceback', '--app', 'HRM.backend.hrm', fixture_file.replace('.json', '')])
