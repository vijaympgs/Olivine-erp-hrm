...
- **⚠️ CRITICAL: Don't create backend APIs for toolbar configuration**

**✅ INSTEAD**: 
- For transaction forms: Use `TransactionToolbar` component (already exists)
- For master pages: Wait for `MasterToolbar` component (coming soon)
- For now: Use simple button groups with standard styling
- **⚠️ CRITICAL: Toolbar configuration MUST come from ERP Menu Item table columns**

### ⚠️ **CRITICAL: Toolbar Architecture (READ THIS FIRST)**

**Toolbar Configuration Source:**
- Toolbar configuration is stored in the `erp_menu_items` table in the database
- Four mode-specific columns: `toolbar_list`, `toolbar_view`, `toolbar_edit`, `toolbar_create`
- Each column contains character codes (e.g., 'NRQFVIOX' for LIST mode)
- This configuration is loaded when the app starts (likely in AppHeader or similar component)

**How Toolbar Works:**
1. Frontend reads toolbar config from already-loaded ERP Menu Item data
2. NO backend API calls for toolbar configuration
3. NO `/api/toolbar-permissions/` endpoint should exist
4. Toolbar is 100% driven by database table columns
...
- **LIST mode**: `toolbar_list` column (e.g., 'NRQFVIOX')
- **VIEW mode**: `toolbar_view` column (e.g., 'X')
- **EDIT mode**: `toolbar_edit` column (e.g., 'SCX')
- **CREATE mode**: `toolbar_create` column (e.g., 'SCX')
