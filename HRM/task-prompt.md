...
- **steering/**: Development standards and guidelines
 ...
 Mindra,
This is toolbar handling for Employee records of HRM,
Need your suggestion, which approach would be best, give me a sccb with the mode props as we implemented in retail
...
If view is pressed in the List mode, the view form is opened, there the toolbar should have,
D - Delete
E - Edit
X - Exit
S - Save

In the view form mode (single record), if edit is pressed, save,clear and exit should be displayed.
Import/export we can implement later
...
If view is pressed in the List mode, the view form is opened, there the toolbar should have, X - Exit only
If edit is pressed (only single record should be selected), the toolbar should display X - Exit, S - Save
If delete is pressed (one/more records can be selected), no view form, it can be handled through dialogue, (now implemented in Employee Records)
Import/export we can implement later
