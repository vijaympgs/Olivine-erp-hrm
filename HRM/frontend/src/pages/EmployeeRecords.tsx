...
export const EmployeeRecords: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<MasterMode>('LIST');
  ...
  const loadData = async (page: number = 1) => {
    await execute(async () => {
      ...
      console.log('[EmployeeRecords] API Response:', resp);
      // Robust handling for both paginated ({ results: [...] }) and flat array ([...]) responses
  ...
  // Toolbar Mode Management (SCCB Compliance)
  const getToolbarMode = (): MasterMode => {
    if (showForm) {
      return editingId ? 'EDIT' : 'CREATE';
    }
    // Employee Records Listing uses LIST mode
    return 'LIST';
  };
  ...
  const handleToolbarAction = (action: string) => {
    switch (action) {
      ...
      case 'profile':
        ...
        break;
      // Delete is NOT in toolbar - handled via row/dialog flow per SCCB
      case 'refresh':
        loadData();
        break;
      case 'search':
        ...
        <div className="m-6 bg-white shadow rounded-lg">
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-medium text-gray-900">
              {viewMode === 'VIEW' ? 'View Employee' : editingId ? 'Edit Employee' : 'Add New Employee'}
            </h3>
          </div>
          <div className="p-6">
            <EmployeeForm
              ...
              onSubmit={async (data: any) => {
                await execute(async () => {
                  ...
                  // Success actions handled by Toolbar usually, but Form might trigger submit directly?
                  // If Toolbar 'Save' triggers handleToolbarAction -> 'save', which calls form submit?
                  // Actually, MasterToolbar architecture relies on Form exposing ref or context?
                  // Or we pass 'onSave' to Form?
                  // The existing code has onSubmit logic here, let's keep it.
                  setShowForm(false);
                  setEditingId(null);
              ...
              mode={viewMode === 'VIEW' ? 'CREATE' : viewMode as 'EDIT' | 'CREATE'}
              hideButtons={true}
        ...
        <>
          ...
          {employees.length === 0 && !isLoading && (
            <div className="p-8 text-center">
              <div className="relative mx-auto w-24 h-24 mb-4 bg-gray-100 rounded-full flex items-center justify-center">
