...
export const MasterToolbar: React.FC<MasterToolbarProps> = ({
    ...
    const isActionDisabled = useCallback((action: ActionButton): boolean => {
        ...
        // 3. Selection-aware enable/disable for LIST mode (SCCB Compliance)
        if (mode === 'LIST') {
            switch (action.id) {
                case 'view':
                case 'edit':
        ...
        // 4. Mode-based toolbar config from database (SCCB Compliance)
        // The toolbar config is fetched from the mode-specific column in erp_menu_items table
        // No hardcoded mode logic - all driven by database
        return false;
    }, [mode, config, selectedCount]);
