...
async function apiCall<T>(
  ...
  // Get token from localStorage (matching useToolbarConfig logic)
  // In a real app this should come from a context or auth service
  ...
  if (token) {
    (headers as any)['Authorization'] = `Token ${token}`; // Note: Backend typically expects 'Token' or 'Bearer'
  }
  ...
  console.log(`[EmployeeService] Fetching: ${url}`);

  try {
    const response = await fetch(url, config);
    console.log(`[EmployeeService] Response Status: ${response.status}`);

    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      console.error(`[EmployeeService] Error:`, errorData);
      throw new ApiError(
    ...
    console.log(`[EmployeeService] Response Data Preview:`, data);
    return data;
  } catch (error) {
    console.error(`[EmployeeService] Catch Error:`, error);
